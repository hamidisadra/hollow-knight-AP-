...
public class StartGameView {
    ...
    public TextButton[] saveSlots = new TextButton[4];
    ...
    private void loadTextures() {
        ...
        titleTexture = new Texture("select_profile.png");
    ...
    private void buildUILayer() {
        ...
        for (int i = 1; i <= 4; i++) {
            saveSlots[i - 1] = new TextButton(i + ".          New Game", skin);

            Table row = createMenuRow(saveSlots[i - 1], pointerTexture);

            TextureRegionDrawable blendedDrawable = new TextureRegionDrawable(new TextureRegion(gameBorderTexture)) {
                @Override
            ...
            row.setBackground(blendedDrawable);
            row.padTop(20).padRight(40);
            gamesTable.add(row).size(700, 90).padBottom(15).row();
        }
        rootTable.add(gamesTable).expand().center().padRight(200).row();
